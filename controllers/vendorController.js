...
exports.createVendor = async (req, res) => {
  try {
   ...
   const companyIds = Array.isArray(company) ? company : (company ? [company] : []);
    ...
    // ── Name duplicate check (company-level) ──
    const nameExists = await Vendor.findOne({
      createdByClient: req.auth.clientId,
      company: { $in: companyIds },
      vendorName: { $regex: new RegExp(`^${vendorName.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i') }
    });
    if (nameExists) {
      return res.status(400).json({ message: "Vendor with this name already exists in this company" });
    }

    // ── Phone duplicate check (company-level) ──
    ...
    // Aisa regex use karein jo +91 (ya koi bhi code) aur 10-12 digits accept kare
...
exports.getVendors = async (req, res) => {
  try {
    ...
    if (perPage && perPage > 100000) {
      console.warn(
        `Very high vendor limit requested: ${perPage}. This may impact performance.`,
      );
    }
...
exports.getVendorBalance = async (req, res) => {
  try {
    ...
    const totalPurchases =
      purchaseAgg.length > 0 ? purchaseAgg[0].totalAmount : 0;
    const cashPurchases =
      purchaseAgg.length > 0 ? purchaseAgg[0].cashAmount : 0;
...
exports.getVendorBalancesBulk = async (req, res) => {
  try {
    ...
    const rows = await Vendor.find(where).select({ _id: 1, balance: 1 }).lean();
...
exports.updateVendor = async (req, res) => {
  try {
    ...
    const up = req.body;
      const companyIds = up.company
      ? (Array.isArray(up.company) ? up.company : [up.company])
      : doc.company.map(String);
    ...
    if (up.vendorName) {
      const nameExists = await Vendor.findOne({
        ...
        company: { $in: companyIds },
        vendorName: { $regex: new RegExp(`^${up.vendorName.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i') }
      });
      if (nameExists) {
        return res.status(400).json({ message: "Vendor with this name already exists in this company" });
      }
    ...
    if (up.contactNumber?.trim()) {
      const phoneExists = await Vendor.findOne({
        createdByClient: req.auth.clientId,
        _id: { $ne: req.params.id },
        company: { $in: companyIds },
        contactNumber: up.contactNumber.trim()
      });
      if (phoneExists) {
        return res.status(400).json({ message: "Contact number already exists in this company" });
      }
    ...
    if (up.gstRegistrationType != null)
      doc.gstRegistrationType = up.gstRegistrationType;
    if (up.pan != null) doc.pan = up.pan;
    if (typeof up.isTDSApplicable === "boolean")
      doc.isTDSApplicable = up.isTDSApplicable;
    if (up.company != null) {
...
exports.downloadImportTemplate = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="vendor_import_template.xlsx"',
    );
...
exports.importVendors = async (req, res) => {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      try {
        ...
        const vendorData = {
          ...
          gstRegistrationType:
            normalizedRow.gstregistrationtype ||
            normalizedRow.gsttype ||
            "Unregistered",
          pan: normalizedRow.pan || "",
          isTDSApplicable:
            (
              normalizedRow.istdsapplicable ||
              normalizedRow.tdsapplicable ||
              "no"
            )
              .toString()
              .toLowerCase() === "yes",
        ...
        if (
          vendorData.gstRegistrationType &&
          gstTypeMapping[vendorData.gstRegistrationType]
        ) {
          vendorData.gstRegistrationType =
            gstTypeMapping[vendorData.gstRegistrationType];
        }
        ...
        if (
          vendorData.gstRegistrationType &&
          !validGstTypes.includes(vendorData.gstRegistrationType)
        ) {
          errors.push(
            `Row ${rowNumber}: Invalid GST registration type "${vendorData.gstRegistrationType}". Must be one of: ${validGstTypes.join(", ")}`,
          );
          continue;
        }
  ...
  createdByClient: req.auth.clientId,
  company: { $in: vendorData.company.length > 0 ? vendorData.company : [''] },
  vendorName: { $regex: new RegExp(`^${vendorData.vendorName.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i') }
});

        if (existingVendor) {
          errors.push(
            `Row ${rowNumber}: Vendor name '${vendorData.vendorName}' already exists`,
          );
          continue;
        }
        ...
        if (error.code === 11000) {
          ...
          errors.push(
            `Row ${rowNumber}: Duplicate ${field} - ${error.keyValue[field]}`,
          );
