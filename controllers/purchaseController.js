    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      try {
        await session.withTransaction(async () => {
          ...
          const netPayable = (typeof req.body.netPayable === "number" && !Number.isNaN(req.body.netPayable))
            ? req.body.netPayable
            : finalTotal;
          ...
          const docs = await PurchaseEntry.create([{
            ...
            paymentMethod,
            advanceReceived: req.body.advanceReceived || 0,
            extraDiscount: req.body.extraDiscount || 0,
            extraDiscountType: req.body.extraDiscountType || "fixed",
          ...
          if (paymentMethod === "Credit") {
            // Update company-specific balance with NET payable (not gross invoice)
            if (!vendorDoc.balances) vendorDoc.balances = new Map();
            const currentBalance = vendorDoc.balances.get(companyId.toString()) || 0;
            vendorDoc.balances.set(companyId.toString(), currentBalance - netPayable); // Negative balance means we owe the vendor
...
exports.updatePurchaseEntry = async (req, res) => {
  try {
    ...
    const originalNetPayable = (typeof entry.netPayable === "number" && !Number.isNaN(entry.netPayable))
      ? entry.netPayable
      : entry.totalAmount;
    ...
    // ensure netPayable is present for balance math
    if (typeof entry.netPayable !== "number" || Number.isNaN(entry.netPayable)) {
      entry.netPayable = entry.totalAmount;
    }
    ...
    const newNetPayable = (typeof entry.netPayable === "number" && !Number.isNaN(entry.netPayable))
      ? entry.netPayable
      : entry.totalAmount; // after update

    if (originalPaymentMethod === "Credit" && newPaymentMethod === "Credit") {
      // Both credit - adjust by difference (negative means we owe more)
      const amountDifference = newNetPayable - originalNetPayable;
      ...
      if (vendorDoc) {
        ...
        const companyIdStr = entry.company.toString();
        const currentBalance = vendorDoc.balances.get(companyIdStr) || 0;
        vendorDoc.balances.set(companyIdStr, currentBalance + originalNetPayable);
        await vendorDoc.save();
      }
    } else if (originalPaymentMethod !== "Credit" && newPaymentMethod === "Credit") {
      ...
      if (vendorDoc) {
        ...
        const companyIdStr = entry.company.toString();
        const currentBalance = vendorDoc.balances.get(companyIdStr) || 0;
        vendorDoc.balances.set(companyIdStr, currentBalance - newNetPayable);
        await vendorDoc.save();
      }
    }
  ...
  try {
    ...
    if (entry.paymentMethod === "Credit") {
      if (vendorDoc) {
        ...
        const companyIdStr = entry.company.toString();
        const currentBalance = vendorDoc.balances.get(companyIdStr) || 0;
        const netPayable = (typeof entry.netPayable === "number" && !Number.isNaN(entry.netPayable))
          ? entry.netPayable
          : entry.totalAmount;
        vendorDoc.balances.set(companyIdStr, currentBalance + netPayable);
        await vendorDoc.save();
      }
    }
