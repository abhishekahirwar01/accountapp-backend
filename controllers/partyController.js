...
async function ensureAuthCaps(req) {
  ...
  if (!req.auth.caps || !Array.isArray(req.auth.allowedCompanies)) {
    ...
    if (!req.auth.allowedCompanies)
      req.auth.allowedCompanies = allowedCompanies;
  }
}
...
exports.createParty = async (req, res) => {
  try {
    ...
    if (!PRIV_ROLES.has(req.auth.role) && !req.auth.caps?.canCreateCustomers) {
      return res
        .status(403)
        .json({ message: "Not allowed to create customers" });
    }
    ...
    if (existingParty) {
      if (existingParty.contactNumber === contactNumber?.trim()) {
        return res
          .status(400)
          .json({ message: "Contact number already exists in this company" });
      }
      if (existingParty.email === email?.trim()?.toLowerCase()) {
        return res
          .status(400)
          .json({ message: "Email already exists in this company" });
      }
    }
    ...
    if (nameExists) {
      return res.status(400).json({ message: "Customer with this name already exists in this company" });
    }
...
exports.getParties = async (req, res) => {
  try {
    ...
    if (total > 50000) {
      console.warn(
        `Large party dataset: ${total} parties for client ${req.auth.clientId}.`,
      );
    }
      ...
      parties.forEach((party) => {
        if (party.balances) {
          if (party.balances instanceof Map) {
            balances[party._id] = Array.from(party.balances.values()).reduce(
              (sum, b) => sum + b,
              0,
            );
          } else if (typeof party.balances === "object") {
            balances[party._id] = Object.values(party.balances).reduce(
              (sum, b) => sum + b,
              0,
            );
          } else {
...
exports.updateParty = async (req, res) => {
  try {
    ...
    const { contactNumber, email, company } = req.body;
    const conditions = [];
    if (contactNumber?.trim()) conditions.push({ contactNumber: contactNumber.trim() });
    if (email?.trim()) conditions.push({ email: email.trim().toLowerCase() });

    if (conditions.length > 0) {
      const duplicateFound = await Party.findOne({
        createdByClient: req.auth.clientId,
        _id: { $ne: req.params.id },
        company: { $in: company || doc.company },
        $or: conditions
      });

      if (duplicateFound) {
        if (contactNumber && duplicateFound.contactNumber === contactNumber.trim()) {
          return res.status(400).json({ message: "Contact number already exists in this company" });
        }
        if (email && duplicateFound.email === email.trim().toLowerCase()) {
    ...
    if (err.code === 11000) {
      return res.status(400).json({ message: "This contact or email is already registered in this company" });
    }
    res.status(500).json({ message: "Server error", error: err.message });
  }
};
...
exports.importParties = async (req, res) => {
  ...
  try {
    ...
    if (records.length === 0) {
      return res
        .status(400)
        .json({ message: "CSV file is empty or could not be parsed." });
    }
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      try {
        ...
        if (
          row.company?.trim() &&
          !mongoose.Types.ObjectId.isValid(row.company.trim().split(";")[0])
        ) {
          const company = await Company.findOne({
            businessName: {
              $regex: new RegExp(`^${row.company.trim()}$`, "i"),
            },
            client: req.auth.clientId,
          });
          if (company) {
            row.company = company._id.toString();
          } else {
            console.warn(
              `Row ${rowNumber}: Company "${row.company.trim()}" not found, importing without company.`,
            );
            row.company = "";
          }
        }
        ...
        const partyData = {
          ...
          gstRegistrationType:
            row.gstRegistrationType?.trim() || "Unregistered",
          pan: row.pan?.trim()?.toUpperCase() || "",
          isTDSApplicable:
            row.isTDSApplicable?.toLowerCase() === "true" ||
            row.isTDSApplicable === "1",
        ...
        if (
          partyData.gstRegistrationType &&
          gstTypeMapping[partyData.gstRegistrationType]
        ) {
          partyData.gstRegistrationType =
            gstTypeMapping[partyData.gstRegistrationType];
        }
        ...
        if (
          partyData.gstRegistrationType &&
          !validGstTypes.includes(partyData.gstRegistrationType)
        ) {
          errors.push(
            `Row ${rowNumber}: Invalid GST registration type "${partyData.gstRegistrationType}". Must be one of: ${validGstTypes.join(", ")}`,
          );
          continue;
        }
        ...
        if (partyData.isTDSApplicable) {
          if (!partyData.tdsSection || partyData.tdsSection.trim() === "") {
            errors.push(
              `Row ${rowNumber}: TDS Section is required when TDS is applicable`,
            );
            continue;
          }
          if (partyData.tdsRate <= 0) {
            errors.push(
              `Row ${rowNumber}: TDS Rate must be greater than 0 when TDS is applicable`,
            );
            continue;
          }
        } else {
        ...
        if (existingParty) {
          if (
            existingParty.contactNumber === partyData.contactNumber &&
            partyData.contactNumber
          ) {
            errors.push(
              `Row ${rowNumber}: Contact number '${partyData.contactNumber}' already exists`,
            );
            continue;
          }
          if (existingParty.email === partyData.email && partyData.email) {
            errors.push(
              `Row ${rowNumber}: Email '${partyData.email}' already exists`,
            );
            continue;
          }
        }
    ...
    return res.status(200).json({
      ...
      importedParties: importedParties.map((p) => ({
        id: p._id,
        name: p.name,
      })),
    });
