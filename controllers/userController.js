    ...
    if (roleId || roleName) {
      ...
      if (isDifferent) {
        if (!canAssignRole(actorRoleDoc, newRole, req.user.role)) {
          return res
            .status(403)
            .json({ message: "Not allowed to assign this role" });
        }
        ...
        if (Array.isArray(permissions) && permissions.length && CAP_KEYS) {
          for (const k of permissions)
            if (CAP_KEYS.includes(k)) extra[k] = true;
        }
        ...
        const existingUPCaps = await UserPermission.findOne({
          client: clientId,
          user: doc._id,
        })
...
exports.getUsersByClient = async (req, res) => {
  try {
    ...
    if (
      actorRoleName !== "admin" &&
      actorRoleName !== "master" &&
      String(myTenant) !== String(clientId)
    ) {
      return res
        .status(403)
        .json({ message: "Not authorized to view users for this client" });
    }
  ...
  try {
    ...
    if (typeof userId !== "string" || typeof password !== "string") {
      return res.status(400).json({
        ...
        details: "User ID and password must be strings",
      });
    }

    const normalizedUserId = userId.trim();

    // Validate userId is not empty after normalization
    if (!normalizedUserId) {
      return res.status(400).json({
        message: "Invalid user ID",
        details: "User ID cannot be empty or contain only whitespace",
      });
    }

    // Find user with populations
    ...
    // Calculate permissions = role.permissions ∪ user.permissions
    ...
    res.json({
      ...
      user: {
        ...
        permissions: perms, // 👈 optional: send permissions to frontend
      },
    });
  } catch (err) {
